"""
Compress chrom 1 of JW, JW with 1000 random alts, 2000 alts, ...and 5000 alts
then calculate the  similarity coefficients. use JWIndel.tar.gz as input data
"""
...
def getBasesData(alts, segmentLength = GAPS_SEGMENT_LENGTH):
    """
    ...
    totalBases = 0
    for alt in alts:
        filename = "/home/tester/data/JW/"+"bases2chr1"+alt
...
def test_bases(alt):
    ...
    n_epochs = 2000 #60000 #55000  before this test
...
if __name__ == '__main__':
    alts = ['alt0', 'alt1000', 'alt2000', 'alt3000', 'alt4000', 'alt5000']
    encoder = test_bases(alts)
    x_dim = encoder.shape[0]
    y_dim = encoder.shape[1]
    z_dim = encoder.shape[2]
    print "encoder shapes: ", x_dim, y_dim, z_dim
    all_indels_num = np.reshape(encoder, (6, x_dim*y_dim*z_dim/6))
    alt0_encoder = all_indels_num[0, :]
    alt1_encoder = all_indels_num[1, :]
    alt2_encoder = all_indels_num[2, :]
    alt3_encoder = all_indels_num[3, :]
    alt4_encoder = all_indels_num[4, :]
    alt5_encoder = all_indels_num[5, :]
    sum  = np.sum(np.square(alt0_encoder))
    sum1 = np.sum(np.square(alt1_encoder - alt0_encoder))
    sum2 = np.sum(np.square(alt2_encoder - alt0_encoder))
    sum3 = np.sum(np.square(alt3_encoder - alt0_encoder))
    sum4 = np.sum(np.square(alt4_encoder - alt0_encoder))
    sum5 = np.sum(np.square(alt5_encoder - alt0_encoder))
    print sum1/sum, sum2/sum, sum3/sum, sum4/sum, sum5/sum
