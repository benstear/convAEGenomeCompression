"""create a convolutional autoencoder w/ Tensorflow for Indel's bases compression,
indel bases are the output of convAutoEncoderBaseswithIndels.py, so have to run that first
...
def autoencoder(input_shape=[None, GAPS_SEGMENT_LENGTH],
                ...
                filter_sizes=[4, 4, 4, 4], #all 4s before
                corruption=False):
...
def getBasesData(filename, segmentLength=GAPS_SEGMENT_LENGTH):
    ...
    """
    twoBasesToNumbers = {'AC': 1.0, 'AG': 2.0, 'AT': 3.0, 'CA': 4.0, 'CG': 5.0, 'CT': 6.0,
                    'GA': 7.0, 'GC': 8.0, 'GT': 9.0, 'TA': 10.0, 'TC': 11.0, 'TG': 12.0,
                    'AA': 13, 'CC':14, 'TT':15, 'GG':16}

    print "Reading data..."
    rawData = []

    rawSNPs = np.load('ref_alt.npy')
    i = 0
    rawData2Bases = []
    totalSNPs = 0
    for base in rawSNPs:
        if i == 0:
            ...
            i = 0
            key = key0 + key1
            if key in twoBasesToNumbers:
                entry = twoBasesToNumbers[key]
                if totalSNPs <= 20:
                    print entry
                rawData2Bases.append(entry)
            else:
                print "Can not find entry with bases: ", key
            totalSNPs += 1
    print "totalSNPs: ", totalSNPs

    rawData = rawData2Bases[:(len(rawData2Bases) - len(rawData2Bases)%segmentLength)]
...
def test_bases():
    ...
    n_epochs = 8000 #60000 #55000  before this test
    ...
    print cost_whendone
    np.save('encodedDataIndels.npy', np.asarray(z_whendone, dtype=np.float16))
    np.save('decoderIndels.npy', np.asarray(w_whendone, dtype=np.float16))
    np.save('errorMatrixIndels.npy', np.asarray(errorMatrix, dtype=np.int8))
